...
const User = function () {
  ...
  this._getLocal = function (fields) {
    ...
    const userDefaultFields = ["id", "username", "firstname", "lastname"];
  ...
  this.retrieveAndStoreCsrfToken = async function () {
    const url =
      this.settings.getDomain().slice(-1) === "/" ? this.settings.getDomain() : `${this.settings.getDomain()}/`;
  ...
  this.getApiClientOptions = async function () {
    return new ApiClientOptions().setBaseUrl(this.settings.getDomain());
  };
